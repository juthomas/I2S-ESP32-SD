...
function App() {
  ...
  // const [uploadStartTime, setUploadStartTime] = useState<Date>(new Date());
  const uploadStartTime = useRef<Date| null>(null);

  const [uploadEstimatedTime, setUploadEstimatedTime] = useState<String>("");
  ...
  const sendFile = () => {
    ...
    xmlHttp.upload.onprogress = (event) => {
      ...
      const actualTime = new Date().getTime() - uploadStartTime.current!.getTime();
      console.log(
      ...
      const secondesRestantes = Math.floor(dureeEstimeeTotale / 1000);
      const minutesRestantes = Math.floor(secondesRestantes / 60);
